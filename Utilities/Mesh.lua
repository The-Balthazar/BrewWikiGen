...
local function FindSCMBoneArrayEndTagOffset(filestring, startoffset)
    return string.find(filestring, 'SKEL\000\000\080\063', startoffset+16) --Catches ~99%
    ...
    or string.find(filestring, '\000SKEL', startoffset+15) + 1 --Catches the remaining 1/16, but has overlap.
end

local function GetArrayFrom0x00DelimitedString(str)
    local things = {}
    string.gsub(str, '([^\000]+)\000', function(thing)
